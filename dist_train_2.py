...
def train(cfg):
    ...
    train_loader2, val_loader2, test_loader, train_sampler = make_data_loader(cfg, args.dataset2, num_workers)
    '''
    ...
    train_loader_iter2 = iter(train_loader2)
    best_IoU= 0
    ...
    for n_iter in range(cfg.train.max_iters):
        if random.random() < 0.5 and round == 0:
            ...
            try:
                _, inputs, labels = next(train_loader_iter2)
            except:
                #train_sampler.set_epoch(n_iter)
                train_loader_iter2 = iter(train_loader2)
                _, inputs, labels = next(train_loader_iter2)
