...
def train(cfg):
    ...
    unsupervised_loader, val_loader, _, unsupervised_sampler = make_data_loader(cfg, args.unsupervised_dataset, num_workers, mask_collate_fn)
    ...
    for n_iter in range(start_iter, cfg.train.max_iters):
        ...
        with torch.cuda.amp.autocast():
            ...
            pred_unsup_l = wetr_r(unsup_imgs_mixed)
            pred_unsup_r = wetr_l(unsup_imgs_mixed)
        ...
        if (n_iter+1) % cfg.train.eval_iters == 0:
            ...
            val_loss, val_score = validate(model=wetr_l, criterion=criterion, data_loader=val_loader_2,cfg = cfg)
            val_loss, val_score_r = validate(model=wetr_r, criterion=criterion, data_loader=val_loader_2,cfg = cfg)
            if args.local_rank==0:
