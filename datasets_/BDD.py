...
class cityscapesDataset(Dataset):
    def __init__(
        ...
        self.root_dir = '/workspace/dataset/BDD/'
        self.stage = stage
        #self.split
        self.img_dir_b = os.path.join(self.root_dir,  'images')
        self.label_dir_b = os.path.join(self.root_dir,  'labels')
        ...
        self.img_dir=[]
        self.label_dir = []
        _splits_dir = os.path.join(self.root_dir, 'ImageSets')
        for splt in self.split:
            with open(os.path.join(os.path.join(_splits_dir, stage + '.txt')), "r") as f:
            ...
            for ii, line in enumerate(lines):
                _image = os.path.join(self.img_dir_b, line + ".jpg")
                #print(_image)
                _cat = os.path.join(self.label_dir_b, line + "_train_id.png")
                #_image = os.path.join(self._image_dir, line)
                #_cat = os.path.join(self._image_dir, line)
                assert os.path.isfile(_image)
                assert os.path.isfile(_cat)
                #self.im_ids.append(line)
                
                self.img_dir.append(_image)
                self.label_dir.append(_cat)
        
        #_splits_dir = os.path.join(self.root_dir, 'ImageSets')

        self.name_list = self.img_dir
