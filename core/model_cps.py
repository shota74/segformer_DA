...
class WeTr(nn.Module):
    def __init__(self, backbone, num_classes=19, embedding_dim=256, pretrained=None):
        super().__init__()
...
class cps(nn.Module):
    def __init__(self, backbone, num_classes=19, embedding_dim=256, pretrained=None):
        ...
        self.WeTr_r = WeTr(backbone=cfg.exp.backbone,
                num_classes=cfg.dataset.num_classes,
                embedding_dim=768,
                pretrained=True)
        
        self.WeTr_l = WeTr(backbone=cfg.exp.backbone,
                num_classes=cfg.dataset.num_classes,
                embedding_dim=768,
                pretrained=True)
    ...
    def forward(self, data, step=1):
        if step == 1:
            return self.WeTr_r(data)
        elif step == 2:
            return self.WeTr_l(data)


if __name__=="__main__":

    pretrained_weights = torch.load('pretrained/mit_b5.pth')
    wetr = WeTr('mit_b5', num_classes=19, embedding_dim=768, pretrained=True)
    wetr._param_groups()
